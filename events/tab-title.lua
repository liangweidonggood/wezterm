...
-- 标签页标题自定义模块，支持显示未见过的标签页计数和自定义图标样式
------------------------------------------------------------------------------------------
...
-- 标签页标题事件配置选项类型定义
-- unseen_icon: 未读输出的图标样式（圆形/带数字圆形/带数字方框）
-- hide_active_tab_unseen: 激活标签页是否隐藏未读指示器
...
local nf = wezterm.nerdfonts -- 访问Nerd Font图标库

local M = {} -- 模块导出表

-- 标签页形状图标
local GLYPH_SCIRCLE_LEFT = nf.ple_left_half_circle_thick --[[  左半圆边框 ]]
local GLYPH_SCIRCLE_RIGHT = nf.ple_right_half_circle_thick --[[  右半圆边框 ]]
local GLYPH_CIRCLE = nf.fa_circle --[[  圆形 ]]

-- 功能状态图标
local GLYPH_ADMIN = nf.md_shield_half_full --[[ 󰞀 管理员权限 ]]
local GLYPH_LINUX = nf.cod_terminal_linux --[[  Linux/WSL ]]
local GLYPH_DEBUG = nf.fa_bug --[[  调试模式 ]]
-- local GLYPH_SEARCH = nf.fa_search --[[  搜索 ]]
local GLYPH_SEARCH = '🔭' -- 搜索图标（使用Emoji替代Nerd Font）
...
-- 标签页颜色配置（不同状态下的背景和前景色）
-- stylua: ignore
local colors = {
   text_default          = { bg = '#45475A', fg = '#1C1B19' },  -- 默认状态文本颜色
   text_hover            = { bg = '#5D87A3', fg = '#1C1B19' },  -- 悬停状态文本颜色
   text_active           = { bg = '#74c7ec', fg = '#11111B' },  -- 激活状态文本颜色

   unseen_output_default = { bg = '#45475A', fg = '#FFA066' },  -- 默认未读输出指示器颜色
   unseen_output_hover   = { bg = '#5D87A3', fg = '#FFA066' },  -- 悬停未读输出指示器颜色
   unseen_output_active  = { bg = '#74c7ec', fg = '#FFA066' },  -- 激活未读输出指示器颜色

   scircle_default       = { bg = 'rgba(0, 0, 0, 0.4)', fg = '#45475A' },  -- 默认半圆边框颜色
   scircle_hover         = { bg = 'rgba(0, 0, 0, 0.4)', fg = '#5D87A3' },  -- 悬停半圆边框颜色
   scircle_active        = { bg = 'rgba(0, 0, 0, 0.4)', fg = '#74C7EC' },  -- 激活半圆边框颜色
}
...
-- @param inset number
-- 创建标签页标题，包含进程名和基础标题，并处理特殊情况（调试模式、搜索模式）
...
function Tab:set_info(event_opts, tab, max_width)
   ...
   self.is_wsl = process_name:match('^wsl') ~= nil  -- 检测是否为WSL
   self.is_admin = (
...
function Tab:update_cells(event_opts, is_active, hover)
   ...
   if is_active then
      tab_state = 'active'  -- 激活状态
   elseif hover then
      tab_state = 'hover'  -- 悬停状态
   end
...
M.setup = function(opts)
   ...
   -- 自定义事件：手动更新标签页名称
   -- 标签页名称将保持锁定状态，直到`reset-tab-title`被触发
